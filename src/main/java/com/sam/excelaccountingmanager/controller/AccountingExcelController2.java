...
@RestController
public class AccountingExcelController2 {

    @PostMapping(path = "/excel/import2")
    public ResponseEntity<byte[]> importAccountingExcel(@RequestPart MultipartFile excel, String date) throws IOException {
        try (InputStream is = excel.getInputStream()) {
            ...
            while (rows.hasNext()) {
                ...
                if (dateCell != null && dateCell.getCellType() == CellType.NUMERIC) {
                    ...
                    if (dateValue.getYear() == LocalDate.now().getYear() && dateValue.getMonth() == Month.JUNE) {
            ...
            headers.add("Content-Disposition", "attachment; filename=filtered-data.xlsx");
